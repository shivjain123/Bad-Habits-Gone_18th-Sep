...
export default class LogBookScreen extends Component {
  ...
  speak=()=>{
    ...
    getAllHabits = async() => {
        this.requestRef = db.collection('habitDetails')
    ...
    renderItem = ({item}) => {
        return(
            <View style={{flexDirection: 'column', justifyContent: 'space-between'}}>
                <Text style={{marginHorizontal: 50, marginVertical: 5}}> {item.habitName} </Text>
                <Text style={{marginHorizontal: 50}}> {"Your Limit is : " + item.limit} </Text>
                <TouchableOpacity
                    style={{ 
                ...
                <Text style={{marginLeft: 200}}> { "You have done your bad habit " + item.counter + " times."} </Text>
            </View>
        )
    }
